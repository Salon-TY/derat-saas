    ...
    <PageContainer className="space-y-4 lg:space-y-6">
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.85fr)]">
        {!isLoading && (stats?.todayInterventions?.length ?? 0) > 0 ? (
          <PageSection title="Prochaines interventions">
          ...
          <PageSection title="Prochaines interventions">
            <div className="rounded-[20px] border border-dashed border-border bg-card/60 px-6 py-10 text-center text-sm text-muted-foreground">
        ...
        <PageSection title="Actions rapides">
          <div className="grid grid-cols-2 gap-3">
            <QuickActionCard
              ...
              description="Planifier un passage"
          ...
          <div className="space-y-3">
            ...
            {!isLoading && !accessLoading && can("contrats") && urgentContracts.length > 0 && (
              <AlertCard icon={AlertTriangle} tone="destructive" href="/contrats">
                <span className="font-semibold text-destructive">
                  {urgentContracts.length} contrat{urgentContracts.length > 1 ? "s" : ""} expirent
            ...
            {!isLoading && !accessLoading && can("contrats") && soonContracts.length > 0 && (
              <AlertCard icon={AlertTriangle} tone="warning" href="/contrats">
                <span className="font-semibold">
                  {soonContracts.length} contrat{soonContracts.length > 1 ? "s" : ""} expirant
                  bientôt
                </span>
                <ul className="mt-2 space-y-1">
            ...
            {!isLoading &&
              !accessLoading &&
              can("programmation") &&
              passagesAProgrammerCount > 0 && (
                <AlertCard icon={CalendarClock} tone="warning" href="/programmation">
                  <span className="font-semibold">
                    {passagesAProgrammerCount} passage{passagesAProgrammerCount > 1 ? "s" : ""} de
                    contrat à programmer
                  </span>
                </AlertCard>
              )}
          </div>
